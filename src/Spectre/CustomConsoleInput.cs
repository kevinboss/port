...
internal class CustomConsoleInput : IAnsiConsoleInput
{
    ...
    public ConsoleKeyInfo? ReadKey(bool intercept) => RewriteConsoleKeyInfo(_input.ReadKey(intercept));
    ...
    {
        if (keyInfo?.Key is not (ConsoleKey.J or ConsoleKey.K)) return keyInfo;
        ...
        {
            ConsoleKey.J => new ConsoleKeyInfo(keyInfo.Value.KeyChar, ConsoleKey.DownArrow, shift, alt, control),
            ConsoleKey.K => new ConsoleKeyInfo(keyInfo.Value.KeyChar, ConsoleKey.UpArrow, shift, alt, control),
